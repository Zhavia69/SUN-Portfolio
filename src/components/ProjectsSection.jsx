...
import APIExplorerDemo from './demos/api/APIExplorerDemo';
import AuthRBACDemo    from './demos/auth/AuthRBACDemo';
import DatabaseLabDemo from './demos/database/DatabaseLabDemo';
import LaravelArchDemo from './demos/laravel/LaravelArchDemo';

/* ─── GitHub deep-links ──────────────────────────────────────────────── */
const GITHUB_BASE = 'https://github.com/Zhavia69/SUN-Portfolio';
const GITHUB_LINKS = {
  0: `${GITHUB_BASE}/tree/main/src/components/demos/api`,
  1: `${GITHUB_BASE}/tree/main/src/components/demos/database`,
  2: `${GITHUB_BASE}/tree/main/src/components/demos/auth`,
  3: `${GITHUB_BASE}/tree/main/src/components/demos/laravel`,
};
...
const ProjectsSection = () => {
  /* Modal state */
  const [openDemo, setOpenDemo] = useState(null); // null | project index

  const openModal  = (idx) => setOpenDemo(idx);
  const closeModal = ()    => setOpenDemo(null);
  ...
  /* ── Currently open project metadata ── */
  const activeProject = openDemo !== null ? projects[openDemo] : null;
  const ActiveDemo    = openDemo !== null ? DEMO_COMPONENTS[openDemo] : null;

  return (
    <>
      {/* ── Demo Modals ── */}
      {projects.map((project, idx) => {
        const Demo = DEMO_COMPONENTS[idx];
        return (
          <DemoModal
            key={idx}
            isOpen={openDemo === idx}
                ...
                <div className="relative flex flex-col h-full">
                  ...
                  <div className="p-8 flex flex-col h-full">
                    ...
                    <div className="flex gap-3 mt-auto">
                      {/* View Details → opens demo modal */}
                      <button
                        onClick={() => openModal(idx)}
                        className={`flex-1 py-2.5 rounded-lg text-sm font-semibold transition-all duration-200 flex items-center justify-center gap-2 bg-gradient-to-r ${project.gradient} text-white hover:opacity-90 active:scale-95 shadow-lg shadow-cyan-500/10`}
                        aria-label={`Open live demo for ${project.title}`}
                      >
                        <ExternalLink className="w-4 h-4" /> View Details
                      </button>

                      {/* Code → GitHub deep-link */}
                      <a
                        href={GITHUB_LINKS[idx]}
                        target="_blank"
                        rel="noopener noreferrer"
                        className="px-4 py-2.5 rounded-lg text-sm font-semibold border border-white/10 text-white bg-white/5 hover:bg-white/10 transition-all duration-200 flex items-center justify-center gap-2"
